...
import { addPaper, FormState } from "../../../../../actions/examPaper";
...
export default function ExamPaperForm() {
    ...
    const [state, formAction, isPending] = useActionState<FormState, FormData>(addPaper, initialState);
    ...
    return (
        ...
        >
            ...
            {["type", "year", "className", "subjectName"].map((field) => (
                <motion.div
                    key={field}
                ...
                >
                    ...
                    >
                        {field === "className" ? "Class" : field === "subjectName" ? "Subject" : field}:
                    </label>
                    <input
                        id={field}
                        type="text"
                    ...
                    {state.errors[field as keyof FormState["errors"]] && (
                        <p className="text-sm text-red-500">{state.errors[field as keyof FormState["errors"]]}</p>
                    )}
                </motion.div>
            ))}
            ...
            <div className="space-y-2">
                ...
                <AnimatePresence>
                    {uploadedUrl && (
                        ...
                        >
                            {uploadedUrl.match(/\.(jpeg|jpg|gif|png)$/) ? (
                                <div className="w-16 h-16 relative rounded-md overflow-hidden border bg-white dark:bg-zinc-800 flex-shrink-0">
                                    <Image fill src={uploadedUrl} alt="Preview" className="object-cover" />
                                </div>
                            ) : uploadedUrl.match(/\.pdf$/) ? (
                            ...
                            <div className="flex-1">
                                ...
                                <p className="text-xs text-indigo-600 dark:text-indigo-400">Upload successful</p>
            ...
            >
                {isPending ? "Creating..." : "🚀 Create Exam Paper"}
            </motion.button>
        </motion.form>
    );
}
